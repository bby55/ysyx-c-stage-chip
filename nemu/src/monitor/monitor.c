...
void analysis_elf(const char *elf_path) {
    ...
    fseek(elf_stream, elf_hdr.e_shoff, SEEK_SET); 
    int found_strtab = 0;
    ...
    symbol_tables = malloc(sym_count * sizeof(symbol_table));
    Assert(symbol_tables, "Memory allocation failed for symbol tables");

    fseek(elf_stream, symtab_hdr.sh_offset, SEEK_SET); 
    Elf32_Sym curr_sym; 
    ...
    for (size_t i = 0; i < sym_count; i++) {
        ...
        if (ELF32_ST_TYPE(curr_sym.st_info) == STT_FUNC && curr_sym.st_size != 0) {
            const char *sym_name = str_table + curr_sym.st_name; 

            strncpy(symbol_tables[i].name, sym_name, sizeof(symbol_tables[i].name) - 1);
            symbol_tables[i].addr = curr_sym.st_value;  
            symbol_tables[i].info = curr_sym.st_info;   
            symbol_tables[i].size = curr_sym.st_size;   
        }
    }

    symbol_tables_size = sym_count;
