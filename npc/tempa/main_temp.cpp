...
// 新增：波形文件指针（全局，用于多函数访问）
static VerilatedVcdC* tfp = NULL;

// 新增：复位周期控制（关键）
#define RESET_TOTAL_CYCLES 10  // 复位持续10个周期
static uint32_t reset_cycle_cnt = 0;      // 复位周期计数器（0~9为复位，≥10时复位结束）
static bool reset_finished = false;       // 复位完成标志（避免重复复位）
...
static bool is_reset = true;  // 初始复位使能（仅用于触发首次复位计数）
...
static uint32_t mrom[MROM_SIZE];
...
extern "C" void ebreak(int exit_code, int exit_pc) {
    ...
    print_iringbuf();
    exit(exit_code); // 按退出码退出，而非固定exit(0)
}
...
extern "C" void mrom_read(int32_t addr, int32_t *data) { 
    ...
    //printf("mrom_read addr: 0x%x\n", addr);
    uint32_t aligned_addr = addr & ~0x3u;
    uint32_t mrom_idx = aligned_addr >> 2;
    if (mrom_idx >= MROM_SIZE) {
...
extern "C" void display(int instr, int pc, int npc, int update_en) {
    int index = (pc>=20000000)?(pc - 0x20000000)/4:0;
    ::instr = mrom[index];
...
bool load_mrom_bin(const char* filename) {
    ...
    size_t count = 0;
    uint32_t word;
    while (count < MROM_SIZE && fread(&word, 1, 4, fp) == 4) {
        mrom[count++] = word;
    }
    fclose(fp);
...
// "c"命令初始化步数统计
static int cmd_c(char *args) {
    start_step = total_steps;       // 保留原步数统计初始化
    is_counting_for_break = true;   // 保留原断点计数标记
    npc_state.state = NPC_RUNNING;  // 保留原运行状态标记
    cpu_exec(UINT64_MAX);           // 核心：调用cpu_exec执行无限步数（等效无限si）
    return 0;
}
...
void prepare_npc_before_state(riscv32_CPU_state &npc_before, int is_nemu, uint32_t n_pc, uint32_t pc, const word_t *gpr) {
    // 重置逻辑：仅当是第一次执行（is_nemu <=1）时，初始化复位状态
    if (is_nemu <= 1) {
        memset(&npc_before, 0, sizeof(riscv32_CPU_state));
        npc_before.pc = 0x20000000; // 与NPC/NEMU初始PC一致
        memcpy(npc_before.gpr, gpr, sizeof(npc_before.gpr)); // 同步初始寄存器
        npc_before.csr = cpu_state.csr; // 同步初始CSR
    } else {
        // 非首次执行：完整拷贝当前NPC状态（PC、GPR、CSR），无多余判断
        memcpy(npc_before.gpr, gpr, sizeof(npc_before.gpr));
        npc_before.pc = (pc == 0)?0x20000000:pc; // 保存执行前的当前PC，而非n_pc
...
bool check_diff_result(const riscv32_CPU_state &npc, const riscv32_CPU_state &ref_nemu, int is_nemu, uint32_t pc) {
    ...
    for (int i = 0; i < 16; ++i) {
        if (npc.gpr[i] != ref_nemu.gpr[i] && npc.gpr[i] != ref_nemu.gpr[i]) {
            if (!has_error) {
...
void cpu_exec(uint64_t n) {
    ...
    uint64_t steps = 0;                // 有效指令执行步数（仅PC变化时计数）
    int cycles = 0;                    // 总时钟周期计数器
    uint64_t remaining_steps = n;      // 复位完成后剩余需要执行的有效步数

    // 核心循环：复位未完成时强制执行 | 复位完成后执行1条且未完成时循环
    while (
        (!reset_finished) ||           // 条件1：复位未完成，持续执行
        (steps < remaining_steps && !ctx->gotFinish() && npc_state.state != NPC_END ) // 条件2：严格限制单次1条
    ) {
        // 保存执行前PC（基准值，固定不变，用于有效指令判断和差分测试）
        uint32_t current_pc_before_exec = cpu_state.pc;
        bool is_valid_instr = false;

#ifdef ENABLE_DIFFTEST
        // 提前准备NPC执行前的基准状态（用于同步NEMU执行前状态）
        prepare_npc_before_state(npc_before, ::is_nemu, ::n_pc, current_pc_before_exec, cpu_state.gpr);
#endif // ENABLE_DIFFTEST

        // 2. 10周期复位控制逻辑（确保复位完成后reset稳定置0）
        if (!reset_finished) {
            top->reset = 1; // 复位阶段强制置1，保证硬件进入复位状态
            if (reset_cycle_cnt < RESET_TOTAL_CYCLES) {
                reset_cycle_cnt++;
            } else {
                reset_finished = true;
                top->reset = 0; // 复位完成后强制置0，释放硬件执行
            }
        } else {
            top->reset = 0; // 非复位阶段保持reset=0，杜绝异常
        }

        // 3. 模拟时钟翻转（驱动硬件执行，标准时钟双沿触发）
        top->clock = 0;
        ctx->timeInc(1);
        top->eval();
        ...
        // 4. 硬件PC同步到软件层（移到分支外，每次时钟循环必同步，保证状态一致）
        // 关键修复：无论是否有效指令，都同步硬件最新PC，避免软件层状态脱节
        cpu_state.pc = ::n_pc;
        // 5. 核心：有效指令判断 + 严格限制单次仅1条（RISC-V 4字节指令步长）
        if (reset_finished && (cpu_state.pc != current_pc_before_exec)) {
            is_valid_instr = true;
            steps++; // 仅计数1条有效指令
            // printf("[DEBUG] 单次有效指令执行完成：PC 0x%x -> 0x%x\n", current_pc_before_exec, cpu_state.pc);

            // 6. 差分测试：NPC与NEMU严格1:1同步（仅复位完成+有效指令时执行）
#ifdef ENABLE_DIFFTEST
            // 拷贝NPC执行后状态（用于后续对比）
            memcpy(npc.gpr, cpu_state.gpr, sizeof(npc.gpr));
            npc.pc = cpu_state.pc; // NPC执行后的PC
            npc.csr = cpu_state.csr;

            // 关键修复1：删除冗余判断，仅保留reset_finished（确保NEMU必执行）
            // 关键修复2：同步NPC【执行前】状态到NEMU，保证执行起点一致
            sync_npc_to_nemu(npc_before); 
            execute_nemu_step(); // NEMU仅执行1条指令，与NPC严格对齐
            get_nemu_result(ref_nemu); // 获取NEMU执行后状态

            // 对比NPC执行后状态 & NEMU执行后状态，不一致则终止
    ...
    // 10. 循环结束后收尾处理（恢复停止状态，打印执行结果）
    is_reset = false;
    if (npc_state.state != NPC_END) {
        npc_state.state = NPC_STOP;
        if (reset_finished) {
            printf("[INFO] 执行完成：共执行%lu条有效指令，总时钟周期%d\n", steps, cycles);
        }
    }
}
...
// sdb_mainloop中累加步数并处理未达断点情况（同步10周期复位逻辑）
void sdb_mainloop() {
    for (char *str; (str = rl_gets()) != NULL; ) {
        ...
        if (npc_state.state == NPC_RUNNING) {
            ...
            while (!ctx->gotFinish() && npc_state.state != NPC_END) {
                ...
                // 核心修改：同步10周期复位控制（与cpu_exec逻辑一致）
                if (!reset_finished) {
                    top->reset = (reset_cycle_cnt < RESET_TOTAL_CYCLES) ? 1 : 0;
                    if (reset_cycle_cnt < RESET_TOTAL_CYCLES) {
                        // printf("[DEBUG] 复位周期: %d/%d\n", reset_cycle_cnt + 1, RESET_TOTAL_CYCLES);
                        reset_cycle_cnt++;
                    } else {
                        reset_finished = true;
                        // printf("[INFO] 10周期复位完成\n");
                    }
                } else {
                    top->reset = 0;
                }
                ...
                ctx->timeInc(1);
                top->eval();
                if (tfp != NULL) tfp->dump(ctx->time()); // 记录低电平时刻信号
                
                // 时钟高电平 - dump波形
                top->clock = 1;
                ctx->timeInc(1);
                top->eval();
                if (tfp != NULL) tfp->dump(ctx->time()); // 记录高电平时刻信号
...
int main(int argc, char** argv) {
    ...
    time(&rtc_timep);
    rtc_tm = gmtime(&rtc_timep);
    const char* hello_rom_path = "/home/ysyxbby/ysyx-workbench/ysyxSoC/char-test";
// 优先加载 hello 程序的 bin 文件，若失败再尝试默认路径（可选）
    // if (!init_rom(hello_rom_path)) {
    // 若 hello 程序加载失败，再加载默认 text.bin（用于降级）
...
//
    ...
    if (first) {
        difftest_memcpy(0x20000000, rom, sizeof(rom), true);
        first = false;
    }
#endif // ENABLE_DIFFTEST
    ...
    // 新增：初始化波形跟踪
    ctx->traceEverOn(true); // 启用波形跟踪功能
    tfp = new VerilatedVcdC; // 创建波形文件对象
    top = new VysyxSoCFull(ctx); // 创建顶层模块实例
    top->trace(tfp, 99); // 关联模块与波形文件，99表示跟踪所有子模块信号
    tfp->open("waveform.vcd"); // 打开波形文件（生成在当前目录）
    printf("[INFO] 波形文件已创建: waveform.vcd\n");
    close_waveform(); // 立即关闭波形文件，停止生成
    printf("[INFO] 波形文件已手动关闭，停止生成\n");
    // 新增：初始化复位计数器和完成标志（关键）
    reset_cycle_cnt = 0;
    reset_finished = false;
    // printf("[INFO] 准备开始10周期复位...\n");
