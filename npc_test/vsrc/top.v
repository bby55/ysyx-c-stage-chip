...
/* verilator lint_on UNOPTFLAT */
  ...
  wire [11:0]instr_type_csr;
  MuxKeyWithDefault #(24, 10, 12) i0 (instr_type_tmp, opcode_funct3, 12'd0, {
    ...
    //12'd36 //ecall
    10'b1110011010, 12'd37, //csrrs
    10'b1110011001, 12'd38  //csrrw
    //.....
  });
  ...
  MuxKeyWithDefault #(2, 32, 12) i8 (instr_type_csr, instr, 12'd0, {
    32'b00000000000000000000000001110011, 12'd36, //ecall
    32'b00110000001000000000000001110011, 12'd39 //mret
  });

  assign instr_type = (instr_type_csr != 0 )?instr_type_csr:
                      (instr_type_nofunct != 0)?instr_type_nofunct:
  ...
  MuxKeyWithDefault #(40, 12, 32) i1 (imm, instr_type, 32'd0, {
    12'd0, I_ex, //
    12'd1, I_ex, //
    12'd2, I_ex, //
    12'd3, S_ex, //
  ...
  MuxKeyWithDefault #(11, 12, 1) i2 (wen, instr_type, 1'd1, {
    12'd3, 1'b0, //SW
    12'd4, 1'b0, //ebreak
    12'd7, 1'b0, //sb
    12'd13, 1'b0, //beq
  ...
  MuxKeyWithDefault #(4, 32, 12) i9 (csr, imm, 12'd0, {
    32'h341, 12'd1, //mepc
    32'h342, 12'd2, //mcause
    32'h300, 12'd3, //mstatus
    32'h305, 12'd4  //mtvec
    //.....
  });

  MuxKeyWithDefault #(4, 12, 32) i10 (t, csr, 32'd0, {
    12'd1, mepc_data, //mepc
    12'd2, mcause_data,//mcause
    12'd3, mstatus_data,//mstatus
    12'd4,  mtvec_data//mtvec
    //.....
  });

  MuxKeyWithDefault #(10, 12, 32) i3 (pc_next, instr_type, pc + 32'd4, {
    ...
    12'd36, mtvec_data, //ecall
    12'd39, mepc_data   //mret
    //.....
  });

  assign csr_val = (instr_type == 12'd37)? t | rs1_data : rs1_data;
  RegisterFile #(
...
module RegisterFile #(ADDR_WIDTH = 1, DATA_WIDTH = 1) (
  ...
  input  [11:0]           instr_type,
  input  [11:0]           csr,
  input  [31:0]           epc,
  input  [31:0]           csr_val,
  output [DATA_WIDTH-1:0] mcause_data,
  output [DATA_WIDTH-1:0] mepc_data,
  output [DATA_WIDTH-1:0] mstatus_data,
  output [DATA_WIDTH-1:0] mtvec_data
);
  ...
  always @(posedge clk) begin
    ...
    mcycle <= mcycle + 1;
    end else if (mcycleh != 32'hFFFFFFFF)begin 
    mcycleh <= mcycleh + 1;
    end else begin
    mcycle <= 0;
    mcycleh <= 0;
    ...
    if(instr_type == 12'd37 || instr_type == 12'd38)begin
      case(csr)
      12'd1: mepc   <= csr_val;
      12'd2: mcause <= csr_val;
      12'd3: mstatus<= csr_val;
      12'd4: mtvec  <= csr_val;
      default: ;
      endcase
    end
...
always @(*) begin
    case (alu_ctrl)
        ...
        12'd37: alu_out = t; //csrrs
        12'd38: alu_out = t; //csrrw
