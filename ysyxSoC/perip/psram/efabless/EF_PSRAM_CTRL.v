...
module PSRAM_READER (
    ...
    wire [7:0]  FINAL_COUNT = in_qpi ? (13 + size*2) : (19 + size*2); // was 27: Always read 1 word
    ...
    wire[1:0] byte_index = in_qpi ? 
                {counter[7:1] - 8'd3}[1:0] :
                {counter[7:1] - 8'd10}[1:0];

    wire [7:0] sample_start = in_qpi ? 8'd14 : 8'd20;
    always @ (posedge clk) begin
        if(counter >= sample_start && counter <= FINAL_COUNT)
            if(sck)
    ...
    assign douten = in_qpi ? (counter < 8) : (counter < 14);
...
module PSRAM_WRITER (
    ...
    wire[7:0]   FINAL_COUNT = in_qpi ? (7 + size*2) : (13 + size*2);
...
module PSRAM_QPI (
    ...
    always @*
        case (state)
            IDLE: if(cmd) nstate = CMD; else nstate = IDLE;
    ...
    assign dout     =   (counter < 8)   ?   {3'b0, CMD_88H[7 - counter]}:4'b0;
