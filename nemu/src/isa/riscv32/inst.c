...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (word_t)src1 >> BITS(imm, 4, 0));

  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , N, R(rd) = (word_t)src1 < (word_t)src2 ? 1 : 0);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = ((word_t)src1 < imm) ? 1 : 0);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , N, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , N, R(rd) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , N, R(rd) = src1 & src2);
