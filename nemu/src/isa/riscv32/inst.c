...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (SEXT(BITS(i, 7, 7),1) << 11) | (BITS(i, 30, 25) << 5) | BITS(i, 11, 8); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , N, R(rd) = (unsigned int)src1 < (unsigned int)src2 ? 1 : 0);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , N, R(rd) = (src1 ^ src2) == 1 ? 0 : 1);
